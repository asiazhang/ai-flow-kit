...
1. 按 `skills/run-release/assets/changelog-template.md` 中的模板，将 `## [VERSION]` 以下内容写入 `CHANGELOG.md` 顶部（顶部已有 `## [Unreleased]` 时，改为将其重命名为新版本条目）
...
git diff --check
bash scripts/check-skills-consistency.sh
```
...
`scripts/check-skills-consistency.sh` 自动校验 README 与 plugin.json 一致。
...
提交并打标签后**不推送**，在本地 review 确认无误后，由用户自行执行：
...
推送前发现问题可直接修复：新增提交，并用 `git tag -d vX.Y.Z` 删除本地 tag 后重新打。
...
由于提交与打标签后不推送，问题在推送前有本地修复窗口：

1. 修复问题
2. 删除并重打本地 tag：`git tag -d vX.Y.Z` 后重新 `git tag vX.Y.Z`（或递增 PATCH 版本号）
3. 重新执行发布步骤

推送后若发现问题，递增 PATCH 版本号重新发布，不删除已推送的 tag，避免强制推送。
